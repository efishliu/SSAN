...
class SSAN(SocialRecommender,GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(SSAN, self).readConfiguration()
        args = config.OptionConf(self.config['SSAN'])
        self.n_layers = int(args['-n_layer'])
        self.ss_rate1 = float(args['-ss_rate1'])
        self.ss_rate2 = float(args['-ss_rate2'])
        self.r_pos = float(args['-r_pos'])
    ...
    def adj_to_sparse_tensor(self,adj):
        ...
        return adj
    ''' 
    def balance_self_supervision(self,em,h_p,h_n):
        user_embeddings = em
        user_sim = tf.matmul(user_embeddings,user_embeddings,transpose_b=True)
        dense_hp = tf.sparse_tensor_to_dense(h_p,validate_indices=False)
        dense_hn = tf.sparse_tensor_to_dense(h_n,validate_indices=False)
        hp_user_sim_all = tf.matmul(dense_hp,user_sim)
        hp_user_sim = tf.diag_part(hp_user_sim_all)
        hn_user_sim_all = tf.matmul(dense_hn,user_sim)
        hn_user_sim = tf.diag_part(hn_user_sim_all)
        balance_loss = tf.reduce_sum(-tf.log(tf.sigmoid(hp_user_sim))+tf.log(tf.sigmoid(hn_user_sim)))
        return balance_loss
      
    def strust_self_supervision(self,em,h_s,h_p,h_n):
        def score(x1,x2):
        ...
        negtivate_neig_embeddings = tf.sparse_tensor_dense_matmul(h_n,user_embeddings)

        pos_sim = score(user_embeddings,positive_neig_embeddings)
        social_sim = score(user_embeddings,social_neig_embeddings)
        neg_sim = score(user_embeddings,negtivate_neig_embeddings)
        struct_loss = tf.reduce_sum(-tf.log(tf.sigmoid(pos_sim-social_sim))-tf.log(tf.sigmoid(social_sim-neg_sim)))
        return struct_loss
    '''
    ...
    def initModel(self):
        super(SSAN, self).initModel()
        print("update H_P")
        print("update status u-v to v-u")
        print("add struct W,b ")
        print("update struct j>p>s to j>s>p")
    ...
    def buildModel(self):
        ...
        total_loss = rec_loss+reg_loss + self.ss_rate1*self.ss_loss1 + self.ss_rate2*self.ss_loss2

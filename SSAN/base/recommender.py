...
class Recommender(object):
    ...
    def evalRatings(self):
        ...
        if self.isOutput:
            outDir = self.output['-dir'] + self.config['dataset'] + '/'
    ...
    def evalRanking(self):
        ...
        outDir = self.output['-dir'] + self.config['dataset'] + '/'
        fileName = self.config['model.name'] + '@' + currentTime + '-measure' + self.foldInfo + '.txt'
        if self.config['model.name'] == 'SSAN':
            model_conf = ['model name:' + self.config['model.name'] + '\n']
            model_conf.append('embedding size:' + str(self.config['num.factors']) + \
                ', epoch num:' + str(self.config['num.max.epoch']) + ', batch_size:' + str(self.config['batch_size']) + '\n')
            SSAN_conf = config.OptionConf(self.config['SSAN'])
            model_conf.append('n_layer:' + str(SSAN_conf['-n_layer']) + ', ss_rate1:' + str(SSAN_conf['-ss_rate1']) + ', ss_rate2:' + str(SSAN_conf['-ss_rate2']) +\
            ', info:' + str(SSAN_conf['-info']) +'\n')
            FileIO.writeFile(outDir, fileName, model_conf)
